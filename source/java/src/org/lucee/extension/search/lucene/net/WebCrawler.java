...
public final class WebCrawler {
    ...
    public void parse(IndexWriter writer, URL current, String[] extensions, boolean recurse, long timeout) throws IOException, PageException {
    	translateExtension(extensions);
    	if(extensions==null || extensions.length==0)extensions=EXTENSIONS;
	...
	private static URL translateURL(URL url) throws MalformedURLException {
		...
		// no dot
		if(dotIndex==-1){
			if(path.endsWith("/")) return e.getHTTPUtil().removeUnecessaryPort(url);
			
			
			return e.getHTTPUtil().removeUnecessaryPort(new URL(
					url.getProtocol(),
					url.getHost(),
					url.getPort(),
    ...
    private static Document toDocument(StringBuffer content,IndexWriter writer, String root, URL current,long timeout) throws IOException, PageException {
    	HTTPResponse rsp = CFMLEngineFactory.getInstance().getHTTPUtil().get(current, null, null, (int)timeout, null, "RailoBot", null,-1,null,null,null);
    ...
    protected static void _parse(Log log,IndexWriter writer, String root, URL current, List<URL> urlsDone, String[] extensions, boolean recurse,int deep,long timeout) throws IOException, PageException  {
    ...
    public static StringBuffer _parseItem(Log log,IndexWriter writer, String root, URL url, List<URL> urlsDone, String[] extensions, boolean recurse,int deep,long timeout) throws IOException, PageException{
    	try{
	...
	protected static void _parseChildren(Log log,StringBuffer content,IndexWriter writer, String root, URL base, List<URL> urlsDone, String[] extensions, boolean recurse,int deep,long timeout) throws IOException  {
    		...
    		if(childIndexer!=null && !childIndexer.isEmpty()){
	    		...
	    		while(it.hasNext()) {
	    			...
	    			if(ci.isAlive()){
	    				ci.interrupt();
	    				if(log!=null)
	    					log.error("WebCrawler","timeout ["+timeout+" ms] occur while invoking page ["+ci.url+"]");
	    			}
	    		}
	...
	private static boolean validExtension(String[] extensions, String file) {
		...
		if(e.getStringUtil().isEmpty(ext))return true;
		for(int i=0;i<extensions.length;i++){
