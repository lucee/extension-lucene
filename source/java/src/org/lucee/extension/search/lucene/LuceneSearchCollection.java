...
public final class LuceneSearchCollection extends SearchCollectionSupport {
	...
	private String getRow(Object column, int row) {
		...
		if(column!=null) return engine.getCastUtil().toString(column,null);
		return null;
	}
    ...
    public SearchResulItem[] _search(SearchData data, String criteria, String language,short type, 
	            	...
	            	while(it.hasNext()) {
		            	...
		            	for(int i=0;i<strLiterals.length;i++) {
		            		...
		            		if(arr.length>0){
				            	...
				            	if(si==null)suggestions.put(strLiterals[i],new SuggestionItemImpl(arr));
				            	else si.add(arr);
		            		}
		            	}
			        }
        ...
        catch (SearchException e) 		{ throw e; }
    ...
    private SpellChecker getSpellChecker(String id) throws IOException, PageException {
    	FSDirectory siDir = FSDirectory.getDirectory(engine.getCastUtil().toFile(_getSpellDirectory(id)));
   ...
   /**
    ...
    private IndexWriter _getWriter(String id,boolean create) throws SearchException, IOException, PageException {
    	...
    	return new IndexWriter(engine.getCastUtil().toFile(dir), SearchUtil.getAnalyzer(getLanguage()), create);
    }
    ...
    private File _getFile(String id,boolean absolute) throws IOException, PageException {
    	Resource res = absolute?engine.getResourceUtil().getFileResourceProvider().getResource(id):_getIndexDirectory(id,true);
	...
	public Object getCategoryInfo() {
		...
		while(it.hasNext()) {
			...
			if(tmp==null) categorytrees.setEL(catTree,engine.getCastUtil().toDouble(1));
			else categorytrees.setEL(catTree,engine.getCastUtil().toDouble(tmp.doubleValue()+1));
			
			// categories
			cats = index.getCategories();
			for(int i=0;i<cats.length;i++) {
				tmp=(Double) categories.get(cats[i],null);
				if(tmp==null) categories.setEL(cats[i],engine.getCastUtil().toDouble(1));
				else categories.setEL(cats[i],engine.getCastUtil().toDouble(tmp.doubleValue()+1));
			}
		}
		return info;
	}
	...
	class ResourceIndexWriter extends IndexWriter {
		...
		public ResourceIndexWriter(Resource dir, Analyzer analyzer, boolean create) throws IOException, PageException {
			super(CFMLEngineFactory.getInstance().getCastUtil().toFile(dir), analyzer, create);
			this.dir=dir;
