...
public final class LuceneSearchCollection extends SearchCollectionSupport {
	...
	private void indexSpellCheck(String id) throws SearchException {
		...
		try {
			...
			// Create analyzer for spell checker (use same analyzer type as your main index)
			Analyzer spellAnalyzer = new StandardAnalyzer(Version.LUCENE_36);

			// Create config for indexing
			IndexWriterConfig config = new IndexWriterConfig(Version.LUCENE_36, spellAnalyzer);
		...
		try {
			...
			if (!criteria.equals("*")) {
				...
				try {
					...
					query = new QueryParser(Version.LUCENE_36, "contents", analyzer).parse(criteria);
				...
				outer: for (int i = 0; i < files.length; i++) {
					...
					if (query == null && "*".equals(criteria)) {
						...
						for (int y = 0; y < len; y++) {
							...
							list.add(createSearchResulItem(highlighter, analyzer, doc, id, scoreDoc.score, // score is
																											// now from
																											// ScoreDoc
									ct, c, aa.getContextPassages(), aa.getContextBytes()));
						}
						// SearchResulItemHits(hits,y,highlighter,analyzer,id,ct,c,aa.getContextPassages(),aa.getContextBytes()));
