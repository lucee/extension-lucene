...
public abstract class SearchCollectionSupport implements SearchCollection {
    ...
    public IndexResult index(PageContext pc, String key, short type, String urlpath, String title, String body, String language, 
        ...
        Lock l = lock();
    	try {
        SearchIndex si = new SearchIndexImpl(title,key,type,query,extensions,language,urlpath,categoryTree,categories,custom1,custom2,custom3,custom4);
        String id=si.getId();
        ...
        if(type==SearchIndex.TYPE_FILE){
        	...
        	// set columns
        		...
        		cols.add("key");
        		cols.add("body");
            	if(!Util.isEmpty(title))cols.add("title");
            	if(!Util.isEmpty(urlpath))cols.add("urlpath");
            	if(!Util.isEmpty(custom1))cols.add("custom1");
            	if(!Util.isEmpty(custom2))cols.add("custom2");
            	if(!Util.isEmpty(custom3))cols.add("custom3");
            	if(!Util.isEmpty(custom4))cols.add("custom4");
        	...
        	else qv = engine.getCastUtil().toQuery(pc.getVariable(query));
            
        	QueryColumn keyColumn=qv.getColumn(key);
            
            String[] strBodies=engine.getListUtil().toStringArrayTrim(engine.getListUtil().toArrayRemoveEmpty(body,","));
    ...
    private QueryColumn getColumnEL(Query query, String column) {
        if(Util.isEmpty(column)) return null;
            ...
            for(int y=0;y<to;y++) {
                ...
                if(si!=null) {
                	...
                	case SearchIndex.TYPE_PATH:
                		...
                		qry.setAt("url",row,toURL(si.getUrlpath(),engine.getStringUtil().replace(file, rootPath, "", true,false)));
                	...
                	case SearchIndex.TYPE_URL:
                		...
                		if(Util.isEmpty(urlPath))urlPath=rootPath;
                		file=record.getKey();
                		qry.setAt("url",row,toURL(urlPath,engine.getStringUtil().replace(file, rootPath, "", true,false)));
                    ...
                    if(Util.isEmpty(title))      qry.setAt("title",row,si.getTitle());
                    if(Util.isEmpty(custom1))    qry.setAt("custom1",row,si.getCustom1());
                    if(Util.isEmpty(custom2))    qry.setAt("custom2",row,si.getCustom2());
                    if(Util.isEmpty(custom3))    qry.setAt("custom3",row,si.getCustom3());
                    if(Util.isEmpty(custom4))    qry.setAt("custom4",row,si.getCustom4());
    ...
    public static String getDirectory(String strUrl) throws MalformedURLException {
    	...
    	// no dot
    	if(dotIndex==-1){
    		if(path.endsWith("/"))return http.removeUnecessaryPort(url).toExternalForm();
    		return http.removeUnecessaryPort(new URL(
    				url.getProtocol(),
    				url.getHost(),
	...
	private static String toURL(String url, String path) {
        if(Util.isEmpty(url)) return path;
        if(Util.isEmpty(path)) return url;
